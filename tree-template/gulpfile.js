...
const thirdPartyLibraries = ['jquery', 'three', 'three-orbitcontrols-ts', 'three-trackballcontrols'];
...
// TODO consider doing it a proper way
// you can import orbit controls on many different ways,
// I use this dependency, because it seems the most recent ones and also the most convenient one
// although there is this issue, which needs fixing, because this dependency bundles its own
// therefore we need to change orbit controls, so it uses *our* version of three js
// this is not required anymore if a new version of this library is released
gulp.task('fix-orbit-controls-three-dependency', function () {
    // var THREE = require("three");
    return gulp.src([path.resolve('node_modules', 'three-orbitcontrols-ts', 'dist', 'index.js')])
        .pipe(replace('var THREE = require("three");', 'var THREE = require("../../three");'))
        .pipe(gulp.dest(path.resolve('node_modules', 'three-orbitcontrols-ts/dist')));
});
...
gulp.task('typescript-to-js', ['fix-orbit-controls-three-dependency'], function () {
    return browserify()
        .add(path.resolve('src', 'ts', 'main', 'main.ts'))
...
gulp.task('clean', function(){
    if(config.production){
        ...
        fs.unlinkSync(path.resolve(paths.dist, paths.js.sideCamUnminified + '.js'));
        fs.unlinkSync(path.resolve(paths.dist, paths.js.sideCamUnminified + paths.js.minifySourceSuffix));
        fs.unlinkSync(path.resolve(paths.dist, paths.js.sideCamUnminified + paths.js.minifySuffix));
    }
});
